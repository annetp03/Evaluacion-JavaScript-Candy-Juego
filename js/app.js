function colorBlink(selector) {  //cambia el color del titulo 
	$(selector).animate({
			opacity: '1',
		}, {
			step: function () {
...
function getRandomInt(min, max) {  // para generar números aleatorios
	min = Math.ceil(min);
	max = Math.floor(max);
...
function columnValidation() { //para validar si hay dulces que se eliminarán en una columna
	for (var j = 0; j < 7; j++) {
		var counter = 0;
		var candyPosition = [];
...
function rowValidation() {   // para validar si hay dulces que deben eliminarse en una fila
	for (var j = 0; j < 6; j++) {
		var counter = 0;
		var candyPosition = [];
		var extraCandyPosition = [];
